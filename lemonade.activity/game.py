import pygame
from pygame.locals import KEYDOWN#, take_input
...
class game:
    ...
    def output(self, text=None):
        ...
        rendered_text = self.font.render("${0:.2f}".format(\
            self.resources[3] / 100.0), True, (0, 0, 0))
        ...
        # Write out the console starting at 500 pixels
                ...
                if (event.type == KEYDOWN):
                    ...
                    if key_name == "backspace":
                        if len(input_text) > 0:
                            ...
                            self.text_lines[len(self.text_lines) - 1] =\
    ...
    def take_input(self, text, default=0):
        ...
        if not in_text:
            ...
            try:
                ...
                if in_text < 0:
                    self.output(\
                        "Entered text should be positive.  Please try again.")
                    return self.take_input(text, default)
            except ValueError:
                self.output(\
                    "Entered text is not a whole number.  Please try again.")
                return self.take_input(text, default)
            else:
                return in_text
