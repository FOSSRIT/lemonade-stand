...
def take_input(text, default=0):
    ...
    if not in_text:
        ...
        try:
            ...
            if in_text < 0:
                frontend.output(\
                    "Entered number must be positive.  Please try again.")
...
def supply_decay():
    ...
    # Let's assume that resource 0 is not perishable
...
def main():
    ...
    for day in range(1, num_days + 1):
        ...
        #Checks to make sure you don't go over your bank amount while buying.
        expenses = 0
        cost_over = 1
        ...
        sales = max(min(game.resources[0], game.resources[1],\
                    game.resources[2]), 0)
        ...
        frontend.output("{0} {1} were made today for ${2:.2f}".format(\
                    sales, variables[1], expenses / 100.0))
        frontend.output("{0} {1} were sold today for ${2:.2f}".format(\
                    sales, variables[1], profit / 100.0))
        frontend.output("You made ${0:.2f} today".format(\
